...
public interface BloomFilter<T> extends Cloneable, Serializable {
  ...
  static Double population(BitSet bitSet, FilterBuilder config) {
    ...
    return -config.size() / ((double) config.hashes()) * Math
        .log(1 - oneBits / ((double) config.size()));
  }
   ...
   * @return {@code true} if the value did not previously exist in the filter. Note, that a false
   * positive may occur, thus the value may not have already been in the filter, but it hashed to a
   * set of bits already in the filter.
   */
   ...
   * @return a list of booleans indicating for each element, whether it was previously present in
   * the filter
   */
  ...
  /**
   * Tests whether an element is present in the filter (subject to the specified false positive
   * rate).
   *
  ...
  /**
   * Return the size of the Bloom filter, i.e. the number of positions in the underlyling bit vector
   * (called m in the literature).
   *
  ...
  /**
   * Returns the expected false positive probability for the expected amounts of elements. This is
   * independent of the actual amount of elements in the filter. Use {@link
   * #getFalsePositiveProbability(double)} for that purpose.
   *
   ...
   * bitwise OR operation on their bit vectors. This operations is lossless, i.e. no elements are
   * lost and the bloom filter is the same that would have resulted if all elements wer directly
   * inserted in just one bloom filter.
   *
   * @param other the other bloom filter
   * @return <tt>true</tt> if this bloom filter could successfully be updated through the union with
   * the provided bloom filter
   */
  boolean union(BloomFilter<T> other);

  /**
   * Performs the intersection operation on two compatible bloom filters. This is achieved through a
   * bitwise AND operation on their bit vectors. The operations doesn't introduce any false
   * negatives but it does raise the false positive probability. The the false positive probability
   * in the resulting Bloom filter is at most the false-positive probability in one of the
   * constituent bloom filters
   *
   * @param other the other bloom filter
   * @return <tt>true</tt> if this bloom filter could successfully be updated through the
   * intersection with the provided bloom filter
   */
  ...
  /**
   * Returns the probability of a false positive (approximated): <br> <code>(1 - e^(-hashes *
   * insertedElements / size)) ^ hashes</code>
   *
   * @param insertedElements The number of elements already inserted into the Bloomfilter
   * @return probability of a false positive after <i>expectedElements</i> {@link #addRaw(byte[])}
   * operations
   */
  ...
  /**
   * Returns the probability of a false positive (approximated) using an estimation of how many
   * elements are currently in the filter
   *
   ...
   *
   * @param n The number of elements already inserted into the Bloomfilter
   * @return probability that a certain bit is zero after <i>expectedElements</i> {@link
   * #addRaw(byte[])} operations
   */
