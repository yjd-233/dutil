...
public final class TimeMap<T> implements Map<T, Long> {
  ...
  public static <T, K> Collector<T, ?, TimeMap<K>> collectMillis(
  ...
  public long putRemaining(T key, long value, TimeUnit timeUnit) {
    long absoluteValue = clock.instant().plusMillis(MILLISECONDS.convert(value, timeUnit))
        .toEpochMilli();
    return compute(key,
        (item, oldValue) -> oldValue == null ? absoluteValue : Math.max(oldValue, absoluteValue));
  }
