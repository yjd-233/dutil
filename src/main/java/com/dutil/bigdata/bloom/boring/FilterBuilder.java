...
import com.dutil.bigdata.bloom.boring.HashProvider.HashFunction;
import com.dutil.bigdata.bloom.boring.HashProvider.HashMethod;
import com.dutil.bigdata.bloom.boring.memory.BloomFilterMemory;
import com.dutil.bigdata.bloom.boring.memory.CountingBloomFilter16;
import com.dutil.bigdata.bloom.boring.memory.CountingBloomFilter32;
import com.dutil.bigdata.bloom.boring.memory.CountingBloomFilter64;
import com.dutil.bigdata.bloom.boring.memory.CountingBloomFilter8;
import com.dutil.bigdata.bloom.boring.memory.CountingBloomFilterMemory;
...
public class FilterBuilder implements Cloneable, Serializable {
  ...
  private static transient Charset defaultCharset = StandardCharsets.UTF_8;
...
//    private int database = Protocol.DEFAULT_DATABASE;
   ...
   * tolerable false positive probability. The size of the BLoom filter in in bits and the optimal
  ...
  /**
   * Constructs a new Bloom Filter Builder using the specified size in bits and the specified number
   * of hash functions.
   *
  ...
  /**
   * @return Return the default Charset used for conversion of String values into byte arrays used
   * for hashing
   */
  ...
  /**
   * Calculates the optimal <i>hashes</i> (number of hash function) given <i>expectedElements</i>
   * (expected number of elements in bloom filter) and <i>size</i> (size of bloom filter in bits).
   *
   * @param n Expected number of elements inserted in the bloom filter
  ...
  /**
   * Calculates the amount of elements a Bloom filter for which the given configuration of size and
   * hashes is optimal.
   *
   * @param k number of hashes
   * @param m The size of the bloom filter in bits.
   * @return amount of elements a Bloom filter for which the given configuration of size and hashes
   * is optimal.
   */
  ...
  /**
   * Sets the number of expected elements. In combination with the tolerable false positive
   * probability, this is used to infer the optimal size and optimal number of hash functions of the
   * filter.
   *
...
//    }
  ...
  /**
   * Sets the tolerable false positive probability. In combination with the number of expected
   * elements, this is used to infer the optimal size and optimal number of hash functions of the
   * filter.
   *
  ...
  /**
   * Sets the number of bits used for counting in case of a counting Bloom filter. For non-counting
   * Bloom filters this setting has no effect. <p><b>Default</b>: 16</p>
   *
  ...
  /**
   * Sets the name of the Bloom filter. If a redis-backed Bloom filter with the provided name exists
   * and it is compatible to this FilterBuilder configuration, it will be loaded and used. This
   * behaviour can be changed by {@link #overwriteIfExists(boolean)}. <p><b>Default</b>: ""</p>
   *
...
//    }
   ...
   * <tt>false</tt></p>
   *
   * @param overwrite boolean indicating whether to overwrite any existing filter with the same
   * name
   * @return the modified FilterBuilder (fluent interface)
   */
  ...
  /**
   * Adds a read slave to speed up reading access (e.g. contains or getEstimatedCount) to normal and
   * counting Redis-backed Bloom filters. The read slave has to be a slave of the main Redis
   * instance (this can be done in the redis-cli using the SLAVEOF command). This setting might
   * cause stale reads since Redis replication is asynchronous. However anecdotally, in our
   * experiments, we were unable to read any stale data - the replication lag between both Redis
   * instances was small than one round-trip time to Redis.
   *
  ...
  /**
   * Sets the method used to generate hash values. Possible hash methods are documented in the
   * corresponding enum {@link HashProvider.HashMethod}. <p><b>Default</b>: MD5</p>
   * <p>
...
//        }
  ...
  /**
   * Constructs a Counting Bloom filter using the specified parameters and by computing missing
...
//        }
  ...
  /**
   * Checks if all necessary parameters were set and tries to infer optimal parameters (e.g. size
   * and hashes from given expectedElements and falsePositiveProbability). This is done
   * automatically.
   *
...
//    }
  ...
  /**
   * @return {@code true} if the Bloom filter that is to be built should overwrite any existing
   * Bloom filter with the same name
   */
  ...
  /**
   * Checks whether a configuration is compatible to another configuration based on the size of the
   * Bloom filter and its hash functions.
   *
