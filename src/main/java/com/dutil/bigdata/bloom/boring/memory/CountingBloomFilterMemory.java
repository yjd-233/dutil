...
import com.dutil.bigdata.bloom.boring.CountingBloomFilter;
import com.dutil.bigdata.bloom.boring.FilterBuilder;
import com.dutil.bigdata.bloom.boring.MigratableBloomFilter;
...
public class CountingBloomFilterMemory<T> implements CountingBloomFilter<T>,
  ...
  public Map<Integer, Long> getCountMap() {
    ...
    for (int i = 0, low = 0, high = low + config().countingBits(); low < counts.length();
        i++, low += config.countingBits(), high += config.countingBits()) {
      long count = 0;
  ...
  public void migrateFrom(BloomFilter<T> source) {
    if (!(source instanceof CountingBloomFilter) || !compatible(source)) {
      throw new IncompatibleMigrationSourceException(
          "Source is not compatible with the targeted Bloom filter");
    }
