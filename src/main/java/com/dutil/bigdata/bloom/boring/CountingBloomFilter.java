...
/**
 * Represents a Counting Bloom Filter, which in contrast to a normal Bloom filter also allow
 * removal.
 */
public interface CountingBloomFilter<T> extends BloomFilter<T> {
  ...
  /**
   * Return the estimated count for an element using the Mininum Selection algorithm (i.e. by
   * choosing the minimum counter for the given element). This estimation is biased, as it doest not
   * consider how full the filter is, but performs best in practice. The underlying theoretical
   * foundation are spectral Bloom filters, see: http://theory.stanford.edu/~matias/papers/sbf_thesis.pdf
   *
  ...
  /**
   * Adds an element and returns its estimated frequency after the insertion (i.e. the number of
   * times the element was added to the filter).
   *
  ...
  /**
   * Removes an element and returns its estimated frequency after the insertion (i.e. the number of
   * times the element was added to the filter).
   *
